// Controls movement during the two hammock positions while keeping both hips aligned.
(function(){
  try{
    ...
    const hipZone = event.zones.hip || {x:638,y:492,w:7,h:1};
    const hipTarget = {x:hipZone.x+hipZone.w/2,y:hipZone.y+hipZone.h/2};
    ...
    // Each pose is ten seconds: five slow, then five fast.
    ...
    function getVisibleBounds(image,sx,sy,sw,sh){
      ...
      try{
        ...
        const result = maxX < 0 ? {x:0,y:0,w:sw,h:sh} : {x:minX,y:minY,w:maxX-minX+1,h:maxY-minY+1};
    ...
    function drawHammockPose(localElapsed){
      const poseOneLength = event.times ? event.times.pose1 : 10000;
      const secondPose = localElapsed >= poseOneLength;
      const local = secondPose ? localElapsed-poseOneLength : localElapsed;

      if(!secondPose){
        const himLean = movement(local,.055,.15);
        drawActorAtHip(players.her.img,'her','down',0,hipTarget.x,hipTarget.y,
          {rotation:Math.PI/2,size:128});
        drawActorAtHip(players.him.img,'him','right',0,hipTarget.x,hipTarget.y,
          {rotation:himLean,size:128,sourceHipOffsetX:.085,sourceHipOffsetY:.065});
        return;
      }

      const herStretch = movement(local,.018,.045);
      drawActorAtHip(players.him.img,'him','down',0,hipTarget.x,hipTarget.y,
        {rotation:Math.PI/2,size:128});
      drawActorAtHip(players.her.img,'her','right',0,hipTarget.x,hipTarget.y,
    ...
    draw = function(){
      ...
      const at = typeof event.timeline === 'function' ? event.timeline() : {pose1At:0,fallAt:20000};
      const inHammockPose = scene.active && elapsed >= at.pose1At && elapsed < at.fallAt;
      ...
      if(inHammockPose) drawHammockPose(elapsed-at.pose1At);
